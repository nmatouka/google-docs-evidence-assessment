...
- Sentences to Check (`Suggestions.gs`, `UI/Suggestions.html`, `POST /evidence/sentences`) is off until someone turns it on for the document. The setting is a DocumentProperty shared by collaborators and records who last changed it; turning it on reads nothing. A scan reads paragraphs and list items before the evidence appendix (tables are skipped), groups them by heading into parts of at most 12,000 characters, and the sidebar sends one part at a time with only the document title and heading. Returned sentences must appear exactly in their paragraph, and sentences that already have an assessment are left out. Results are cached per author for six hours, keyed by each part's text, so a rescan sends only parts that changed. "Show in document" selects a sentence; "Assess" anchors it as a pending claim and opens Assess Selected Text. Nothing in the document changes.
- Repeating a search (same document, claim, and context, or the same edited search) or a labeling request (same claim and passages) within six hours reuses the result from the author's own script cache (`readUserCache`/`writeUserCache` in `Utils.gs`, keyed by `digestOf` the request) instead of calling Climateshed, so reopening a claim costs nothing. The panel says when a search was reused. Failures aren't cached.
...
## What the AI may and may not do (evidence search, source labels, rating suggestions, and sentence suggestions built)
